...
const AnalyticsDimensionSecond = ({
  setPageNumber,
  ...
  const { sharedAnalyticsDimensionData, setAnalyticsDimensionSharedData } =
    useContext(DataContext);
  const initialData = filterPopupData?.analytics_dimension2?.map((v, i) => ({
    ...v,
    dimension_id: i + 1,
  }));
  ...
  const handleCheck = (item) => {
    ...
    [...updatedDimensions].sort((a, b) => {
      ...
      if (a.pin_key && b.pin_key) {
        return (
          analyticsDimensionValueBox.findIndex((item) => item.name === a.name) -
          analyticsDimensionValueBox.findIndex((item) => item.name === b.name)
        );
      }
      return 0;
    });
    setanalyticsDimensionValueBox(updatedDimensions);
    setAnalyticsDimensionSharedData({ columns: updatedDimensions });
    setDimensionMatrix(
      updatedDimensions?.map((item) => item.dimension_checked)
    );
  ...
  const onDragEnd = (result) => {
    ...
    const firstHalf = allDimensionData.filter(
      (dim) => dim.dimension_checked && dim.pin_key
    );
    const secondHalf = allDimensionData.filter(
      (dim) => dim.dimension_checked && !dim.pin_key
    );
  ...
  const handlePinClick = (value) => {
    ...
    const updatedDimensions = analyticsDimensionValueBox?.map((dimension) => {
  ...
  useEffect(() => {
    setAnalyticsDimensionSharedData({ columns: analyticsDimensionValueBox });
    setanalyticsDimensionValueBox(analyticsDimensionValueBox);
  }, [Data?.length > 0]);
  return (
                ...
                {filterDimensions(true, true).map((dimension, index) => (
                  ...
                  >
                    {(provided) => (
                      ...
                      >
                        <div
                          className="omit-class"
                          onClick={(e) => handleCheck(dimension)}
                        >
                          <div
                            {...provided.dragHandleProps}
                            className={`padding-non-draggable`}
                          >
                            {dimension?.dimension_checked && (
                              <RiDraggable
                                className="drag-icon"
                          ...
                          <div className="pin-item-box">
                            <div className="pin-icon">
                              {!dimension.pin_key ? (
                                <BsPin
                                  onClick={() =>
                                    handlePinClick(dimension.data_column_id)
                                  }
                                  className="pin-empty"
                                />
                              ) : (
...
export default AnalyticsDimensionSecond;
