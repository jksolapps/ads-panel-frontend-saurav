...
// import ColumnDropdown from '../AppDetails/Filters/ColumnDropdown';
// import SecondColumnFilter from '../AppDetails/Filters/SecondColumnFilter';
...
function weekKeyToSortInt(weekKey) {
...
const PerformanceTable = () => {
  ...
  const {
    ...
    'analytics-report-from-aggregated-tables',
    formData,
    { staleTime: 60 * 1000, refetchOnMount: 'ifStale' }
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener('app-visibility-changed', handleVisibilityChange);
    return () => window.removeEventListener('app-visibility-changed', handleVisibilityChange);
  }, [queryClient]);
  ...
  const postToWorker = (aaData, searchText) => {
    if (!workerRef.current) return;
    setIsLoaderVisible(true);
    reqIdRef.current += 1;
    workerRef.current.postMessage({
  ...
  useEffect(() => {
    ...
    if (apiResponse?.status_code == 1) {
      const aaData = apiResponse?.aaData || [];
      setSearchData(aaData);
      postToWorker(aaData, searchText);
    } else {
      setSingleReportData([]);
      setCsvData([]);
      setTotalRecordsData(null);
  ...
  useEffect(() => {
    const base = searchData?.length ? searchData : [];
    if (base.length === 0) {
      setSingleReportData([]);
      setCsvData([]);
      setTotalRecordsData(null);
  ...
  const handleSearch = (e) => {
    ...
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    timeoutRef.current = setTimeout(() => {
      if (workerRef.current && searchData?.length > 0) postToWorker(searchData, query);
    }, 350);
  };
  ...
  const handleSearchClose = () => {
    setSearchText('');
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    if (workerRef.current && searchData?.length > 0) postToWorker(searchData, '');
  };
  ...
  const dimensionHeaderWidth = (dim) => {
    switch (dim) {
      ...
      case 'AD_UNIT': return 130;
      case 'COUNTRY':
      case 'FORMAT': return 140;
      case 'MOBILE_OS_VERSION':
      case 'GMA_SDK_VERSION': return 195;
      case 'APP_VERSION_NAME': return 155;
      case 'ALL_APPS': return 280;
      default: return 110;
    }
  };
  ...
  const dimensionValueLabel = (dim, row) => {
    switch (dim) {
      case 'MONTH':
        return moment(String(row.report_date ?? ''), 'YYYYMM').format('MMM YY') || '-';
      case 'WEEK':
        return formatWeekKey(row.report_date);
      case 'YEAR': {
        const val = String(row.report_date ?? '');
        if (/^\d{4}$/.test(val)) return val;
        const m = moment(val, val.length === 6 ? 'YYYYMM' : 'YYYYMMDD');
        return m.isValid() ? m.format('YYYY') : '-';
      }
      case 'AD_UNIT': return row.au_display_name ?? '-';
      case 'COUNTRY': return row.country_name ?? '-';
      case 'FORMAT': return row.au_format ?? '-';
      case 'MOBILE_OS_VERSION': return row.mobile_os_version ?? '-';
      case 'GMA_SDK_VERSION': return row.gma_sdk_version ?? '-';
      case 'APP_VERSION_NAME': return row.app_version ?? '-';
      default: return '-';
    }
  };
  ...
  const dimensionSortValue = (dim, row) => {
    switch (dim) {
      case 'MONTH': return Number(row.report_date) || 0;
      case 'WEEK': return weekKeyToSortInt(String(row.report_date ?? ''));
      case 'YEAR': return Number(row.report_date) || 0;
      case 'GMA_SDK_VERSION':
        return versionToSortableInt(extractVersionParts(row.gma_sdk_version));
      case 'MOBILE_OS_VERSION':
        return versionToSortableInt(extractVersionParts(row.mobile_os_version));
      ...
      default: return String(dimensionValueLabel(dim, row) ?? '').toLowerCase();
    }
  };
  ...
  const tanstackColumns = useMemo(() => {
    ...
    const renderWeekCell = (row) => {
      const label = formatWeekKey(row.original?.report_date);
      const start = row.original?.week_start;
      const end = row.original?.week_end;

      return (
        <div className="report_column_box custom_word_ellipsis">
          <div className="report_main_value" title={label}>{label}</div>
        </div>
      );
    };
    ...
    const firstCol = {
      ...
      cell: ({ row }) => {
        ...
        if (firstColumnDimension === 'WEEK') return renderWeekCell(row);
        const v = dimensionValueLabel(firstColumnDimension, row.original);
        return (
          <div className="report_column_box custom_word_ellipsis">
            <div className="report_main_value" title={v}>{v}</div>
          </div>
        );
      },
      size: dimensionHeaderWidth(firstColumnDimension),
    ...
    const secondCol = {
      ...
      cell: ({ row }) => {
        ...
        if (secondColumnDimension === 'WEEK') return renderWeekCell(row);
        const v = secondColumnDimension
          ? dimensionValueLabel(secondColumnDimension, row.original)
          : '(not set)';
        return (
      ...
      size: secondColumnDimension ? dimensionHeaderWidth(secondColumnDimension) : 130,
      meta: { alignMent: 'center', headerClassName: 'custom_report_column', omit: !secondColumnDimension },
      sortingFn: 'basic',
    };
    ...
    return [
...

      {
        ...
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_cost, minWidth: 125 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.AD_COST },
      },

      {
        ...
        accessorFn: (row) => parseMoney(row.estimated_earnings),
        header: () => totalRecordsData ? (
          <Tippy content={'Actual Earnings'} placement="top" duration={0} offset={[0, 0]} className="custom_black_tippy">
            <div className="report-title custom_right_head">
              <div className="top_total">
                <div className="report-header hover_tooltip_box title_tooltip"><span>Earnings</span></div>
                <div className="report-total">{indianNumberFormat(totalRecordsData.total_estimated_earnings)}</div>
              </div>
            </div>
          </Tippy>
        ) : <div className="report-title custom_right_head">Earnings</div>,
        cell: ({ row }) => <div className="report_column_box">{row.original?.estimated_earnings}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_estimated_earnings, minWidth: 125 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.ESTIMATED_EARNINGS },
      },

      {
        id: 'ROAS',
        accessorFn: (row) => { const c = parseMoney(row.cost); const r = parseMoney(row.estimated_earnings); return c ? r / c : 0; },
        header: () => totalRecordsData ? (
          <Tippy content={'Actual ROAS'} placement="top" duration={0} offset={[0, 0]} className="custom_black_tippy">
            <div className="report-title custom_right_head">
              <div className="top_total">
                <div className="report-header hover_tooltip_box title_tooltip"><span>A.ROAS</span></div>
                <div className="report-total">{totalRecordsData?.total_roas}</div>
              </div>
            </div>
          </Tippy>
        ...
        cell: ({ row }) => {
          const c = parseMoney(row.original?.cost); const r = parseMoney(row.original?.estimated_earnings);
          return <div className="report_column_box">{c ? (r / c).toFixed(2) : '0.00'}</div>;
        },
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_roas, minWidth: 100 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.ROAS },
      },

      {
        ...
        accessorFn: (row) => parseMoney(row.fsr),
        header: () => totalRecordsData ? (
          <Tippy content={'Cumulative Earning'} placement="top" duration={0} offset={[0, 0]} className="custom_black_tippy">
            <div className="report-title custom_right_head">
              <div className="top_total">
                <div className="report-header hover_tooltip_box title_tooltip"><span>C.Earning</span></div>
                <div className="report-total">{indianNumberFormat(totalRecordsData?.total_fsr)}</div>
              </div>
            </div>
          </Tippy>
        ) : <div className="report-title custom_right_head">C.Earnings</div>,
        cell: ({ row }) => <div className="report_column_box">{indianNumberFormat(row.original?.fsr)}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_fsr, minWidth: 140 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.CUMULATIVE_EARNINGS },
      },

      {
        id: 'CUMULATIVE_ROAS',
        accessorFn: (row) => { const c = parseMoney(row.cost); const r = parseMoney(row.fsr); return c ? r / c : 0; },
        header: () => totalRecordsData ? (
          <Tippy content={'Cumulative ROAS'} placement="top" duration={0} offset={[0, 0]} className="custom_black_tippy">
            <div className="report-title custom_right_head">
              <div className="top_total">
                <div className="report-header hover_tooltip_box title_tooltip"><span>C.ROAS</span></div>
                <div className="report-total">{totalRecordsData?.total_cumulative_roas}</div>
              </div>
            </div>
          </Tippy>
        ...
        cell: ({ row }) => {
          const c = parseMoney(row.original?.cost); const r = parseMoney(row.original?.fsr);
          return <div className="report_column_box">{c ? (r / c).toFixed(2) : '0.00'}</div>;
        },
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_cumulative_roas, minWidth: 100 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.CUMULATIVE_ROAS },
      },

      {
        id: 'PROFIT',
        accessorFn: (row) => parseMoney(row.estimated_earnings) - parseMoney(row.cost),
        header: () => totalRecordsData ? (
          <div className="report-title custom_right_head">
            <div className="top_total">
              <div className="report-header">Profit</div>
        ...
        cell: ({ row }) => {
          const profit = parseMoney(row.original?.estimated_earnings) - parseMoney(row.original?.cost);
          return (
            <div className="report_column_box">
              {(profit < 0 ? '- $' : '$') + indianNumberFormat(Math.abs(profit.toFixed(2)))}
            </div>
        ...
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_profit, minWidth: 170 }),
        meta: { alignMent: 'right', isDynamic: true, omit: !columnVisibility.PROFIT },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('eCPM', indianNumberFormat(totalRecordsData.total_observed_ecpm))
          : <div className="report-title custom_right_head">eCPM</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.observed_ecpm}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_observed_ecpm, minWidth: 90 }),
        meta: { alignMent: 'right', omit: !columnVisibility.IMPRESSION_RPM, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('Requests', indianNumberFormat(totalRecordsData.total_ad_requests))
          : <div className="report-title custom_right_head">Requests</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.ad_requests}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_ad_requests, minWidth: 100 }),
        meta: { alignMent: 'right', omit: !columnVisibility.AD_REQUESTS, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData ? (
          <div className="report-title custom_right_head">
            <div className="top_total">
              <div className="report-header">Match rate</div>
              <Tippy content={indianNumberFormat(totalRecordsData?.total_matched_requests)} placement="top" duration={0} offset={[0, 2]} className="custom_black_tippy">
                <div className="report-total">{totalRecordsData?.total_match_rate}</div>
              </Tippy>
            </div>
          </div>
        ...
        cell: ({ row }) => (
          <div className="report_column_box custom_right_head hover_tooltip_box">
            <Tippy content={row.original?.matched_requests} placement="top" duration={0} offset={[0, 2]} className="custom_black_tippy">
              <span>{row.original?.match_rate}</span>
            </Tippy>
          </div>
        ),
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_matched_requests, minWidth: 120 }),
        meta: { alignMent: 'right', omit: !columnVisibility.MATCH_RATE, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('Impressions', indianNumberFormat(totalRecordsData.total_impressions))
          : <div className="report-title custom_right_head">Impressions</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.impressions}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_impressions, minWidth: 100 }),
        meta: { alignMent: 'right', omit: !columnVisibility.IMPRESSIONS, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('Show rate', totalRecordsData.total_show_rate)
          : <div className="report-title custom_right_head">Show rate (%)</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.show_rate}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_show_rate, minWidth: 120 }),
        meta: { alignMent: 'right', omit: !columnVisibility.SHOW_RATE, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('Clicks', indianNumberFormat(totalRecordsData.total_clicks))
          : <div className="report-title custom_right_head">Clicks</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.clicks}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_clicks, minWidth: 100 }),
        meta: { alignMent: 'right', omit: !columnVisibility.CLICKS, isDynamic: true },
      },

      {
        ...
        header: () => totalRecordsData
          ? headerWithTotal('CTR', totalRecordsData.total_impression_ctr)
          : <div className="report-title custom_right_head">CTR (%)</div>,
        cell: ({ row }) => <div className="report_column_box custom_right_head">{row.original?.impression_ctr}</div>,
        size: dynamicColumnWidthCal({ value: totalRecordsData?.total_impression_ctr, minWidth: 100 }),
        meta: { alignMent: 'right', omit: !columnVisibility.IMPRESSION_CTR, isDynamic: true },
      },
    ];
  }, [
    firstColumnDimension, secondColumnDimension, availableFilters,
    isOpen, secondIsOpen, secondArrow, fetchFlag, columnVisibility, totalRecordsData,
  ]);
  ...
  const applyModalColumns = (updatedCols) => {
    const updatedVisibility = {};
    updatedCols.forEach((col) => { updatedVisibility[col.sortValue] = !col.omit; });
    setColumnVisibility((prev) => ({ ...prev, ...updatedVisibility }));
  };
  ...
  const MAX_ROWS = 18;
  const MIN_HEIGHT = 300;
  const tableHeight = Math.max(MIN_HEIGHT, Math.min(singleReportData.length, MAX_ROWS) * ROW_HEIGHT) + 80;
   ...
   const time = apiResponse?.updated_at;

  const updatedAtLabel = useMemo(() => getRelativeTime(time), [time, count]);

  return (
    <div className={`table-box-wrap main-box-wrapper pdglr24 report-table-box ${!singleReportData.length ? 'no_data_table_wrap' : ''}`}>
      <div className="userBoxWrap user-section-wrapper">
        <div className="popup-full-box form-box-wrap form-wizard">
          <div className="popup-box-wrapper">
            <div
              className={`box-wrapper table-container ${secondColumnDimension ? 'extra_column_visible' : ''}`}
              style={{ zIndex: 8 }}
            >
              {showOverlayLoader && (
                <div className="shimmer-spinner overlay-spinner">
              ...
              <div className="custom-search-filter single_app_search">
                <div className="single_app_search_inner">
                  <input value={searchText} onChange={handleSearch} placeholder="Search..." autoComplete="off" />
                  {searchText?.length > 0 && <MdClose className="search-close" onClick={handleSearchClose} />}
                </div>

                <div className="more-button three-icon-button more_btn_box">
                  <span className="time-stamp">{'(' + 'updated ' + updatedAtLabel + ')'}</span>
                  {singleReportData && <div className="row_count">Total rows : {singleReportData?.length}</div>}
                  <MdMoreVert className="material-icons" />
                  <div className="more-box w-250">
                    <div className="border-box">
                      {csvData?.length > 0 && (
                        <CSVLink className="downloadbtn" filename="all-report.csv" data={csvData}>
                          <span className="material-icons"><FiDownload /></span>
                          Download CSV
                        </CSVLink>
                      )}
                    </div>
                    <div className="border-box more_btn_item" onClick={() => setOmitModalShow(true)}>
                      <span className="material-icons"><IoEyeOff /></span>
                      <span>Hide/Show column</span>
                    </div>
                  </div>
                </div>
              ...
              <GeneralTanStackTable
                ...
                overscan={10}
                stickyColumns={secondColumnDimension ? 2 : 1}
                defaultSortColumn={`FirstColumn_${firstColumnDimension}`}
                defaultSortDirection="desc"
              />
            </div>
          </div>
