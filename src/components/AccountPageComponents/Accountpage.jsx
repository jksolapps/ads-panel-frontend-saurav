...
const Accountpage = () => {
    ...
    totals[row_type].total_previous_month += totalPreviousMonthCount;
		...
		const total_month = (totals?.Revenue?.total_month_original / totals?.Impressions?.total_month_original) * 1000;
		// date wise
				...
				<div className='userBoxWrap user-section-wrapper'>
					...
					<>
						<div className={`Account-Table ${defaultShortValue}`}>
							...
							<AccountInfoModal show={updateProfileShow} onHide={() => setUpdateProfileShow(false)} />
						</div>
					</>
				</div>
			</div>
