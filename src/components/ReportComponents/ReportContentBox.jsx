...
const ReportContentBox = () => {
		...
		const getHighlightedTotal = (current, previous, isPercentage = false) => {
			...
			return (
				<>
					...
					<span className={`report_prev_total ${highlightClass}`}>({indianNumberFormat(previous)})</span>
				</>
			);
		};
		...
		return [
...

			{
				...
				cell: ({ row }) => {
					...
					if (groupFilterKey === 'WEEK') {
						...
						const monthNum = (moment(r?.report_date, 'YYYYMM').month() + 1).toString().padStart(2, '0');
...

			{
				...
				cell: ({ row }) => {
					...
					const previous = r?.prev_impression_ctr === '-' ? '0.00%' : r?.prev_impression_ctr || '0.00%';
	...
	const baseFilterOrder = useMemo(() => {
		return appVersion?.length > 0
			...
			: ['AppPopup', 'FormatPopup', 'AdUnitsPopup', 'CountryPopup', 'PlatformPopup', 'GroupByPopup'];
	}, [appVersion?.length]);
