...
const ColumnDropdown = ({
  ...
  const firstColumnDropdown = availableFilters?.filter((filter) => {
    if (
    ...
    if (secondColumnDimension === "MONTH") {
      return filter.api_Value !== "WEEK" && filter.api_Value !== "YEAR";
    } else if (secondColumnDimension === "WEEK") {
      return filter.api_Value !== "MONTH" && filter.api_Value !== "YEAR";
    } else if (secondColumnDimension === "YEAR") {
      return filter.api_Value !== "MONTH" && filter.api_Value !== "WEEK";
    }

    return true;
  });

  const firstColumnPlusDropdown = availableFilters?.filter((filter) => {
    if (firstColumnDimension == "MONTH") {
      return (
        filter.api_Value !== firstColumnDimension &&
        filter.api_Value !== "WEEK" &&
        filter.api_Value !== "YEAR"
      );
    } else if (firstColumnDimension == "WEEK") {
      return (
        filter.api_Value !== firstColumnDimension &&
        filter.api_Value !== "MONTH" &&
        filter.api_Value !== "YEAR"
      );
    } else if (firstColumnDimension == "YEAR") {
      return (
        filter.api_Value !== firstColumnDimension &&
        filter.api_Value !== "MONTH" &&
        filter.api_Value !== "WEEK"
      ...
      return filter.api_Value !== firstColumnDimension;
    }
  });
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            >
              {firstColumnDropdown?.length > 0 ? (
                firstColumnDropdown?.map((filter) => (
                  <div
                    key={filter.key}
                    onClick={(e) => {
                      e.stopPropagation();
      ...
      {secondIsOpen && (
        <div className="second-column-btn-inner">
          ...
          >
            ...
            >
              {firstColumnPlusDropdown?.length > 0 ? (
                firstColumnPlusDropdown?.map((filter) => (
                  <div
                    key={filter.key}
                    onClick={(e) => {
                      e.stopPropagation();
