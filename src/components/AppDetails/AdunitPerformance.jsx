		...
		categories.forEach((category) => {
			if (data[category]) {
				data[category] = data[category]?.map((app, index) => {
					if (app) {
						const dataPointsCurrent = app?.date_wise_revenue_current?.map(([date, revenue], index) => ({
							label: formatDate(date),
							y: microValueConvert(revenue),
							x: index,
						}));
						const dataPointsCompare = app?.date_wise_revenue_compare?.map(([date, revenue], index) => ({
							label: formatDate(date),
							y: microValueConvert(revenue),
							x: index,
						}));
		...
		>
			...
			<div className='table-wrap app-overview-table'>
				{showMainLoader ? (
					...
					<div className='scroll-container' style={{ overflowY: 'auto', minHeight: computedMaxHeight, maxHeight: computedMaxHeight }}>
                    ...
                    <div className="metric-header-wrap">
                      <span className="metric-main-label" onClick={() => setShowImpressionDropdown((prev) => !prev)}>
                  ...
                  {extraMetric && (
                    <th>
                      <div className="metric-header-wrap">
                        <span className="metric-main-label" onClick={() => setShowExtraDropdown((prev) => !prev)}>
							...
							<tbody>
								{adUnitPerformanceData[performanceSelect]?.map((data, index) => (
									...
									>
										...
										>
											<h5 className='mb-1 fw-normal'>{data?.impr} </h5>
										</td> */}
