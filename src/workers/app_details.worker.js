...
function filterDataByDimensions(data, firstDimension, secondDimension, searchValue) {
	...
	if (firstDimension?.includes('YEAR') || secondDimension?.includes('YEAR')) {
		...
		for (const row of data) {
            ...
            cur.arpu += parseIntSafe(row.arpu);
			cur.arpdau += parseMoney(row.arpdau);
			cur.dau_av += parseIntSafe(row.dau_av);
		...
		data = Array.from(yearMap.values()).map((d) => {
			const observed = d.impressions > 0 ? (d.estimated_earnings / d.impressions) * 1000 : 0;
				...
				const avgDauAv = d.count > 0 ? d.dau_av / d.count : 0;
			...
			return {
                ...
                arpu: indianNumberFormat(d.arpu),
				...
				arpdau: `$${indianNumberFormat((d.arpdau / d.count).toFixed(4))}`,
dau_av: indianNumberFormat(avgDauAv),
				av_rate: d.dau_av > 0 ? `${(d.dau_av / d.active_users).toFixed(2) * 100}%` : '0.00%',
...
function totalRecordsSum(data) {
	...
	acc.total_impr_per_user = acc.total_active_users > 0 ? ( acc.total_impressions / acc.total_active_users ).toFixed(2) : "0.00";
	acc.total_arpdau = `$${Number(acc.total_arpdau).toFixed(4)}`;
acc.total_dau_av = acc.total_dau_av.toFixed(2);
